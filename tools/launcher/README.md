...
```bash
engine/.venv/bin/python tools/launcher/make-icon.py   # only if icon.png is missing
...
**It is not a packaged app.** A real signed `.app` is Phase 5 and carries three
open questions: bundling the Python engine (a venv today), whether PDF.js's
WASM/cmap assets load under `file://`, and code signing. See `ROADMAP.md`.
