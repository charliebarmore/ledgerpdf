...
Written for a pilot tester. It covers what the installer does, what Windows may
say about it, and what that actually means — because the honest answer to a
security prompt is not "click Run anyway".

**These builds are unsigned.** Read the section on the security prompt before
...
There is no public download yet. If you have access to the repository, open the
**Actions** tab → **Windows x64** → **Run workflow**, leave `package` ticked, and
download `ledgerpdf-win-installer-UNSIGNED` when it finishes (~7 minutes).
